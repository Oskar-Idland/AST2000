...
class Particle:
    ...
    def advance(self, timestep=1e-13):
        ...
        axis = np.nonzero(abs(self.position[-2]) >= self.box.length/2)  # Checking for each dimension whether the position is outside the length of the box
        if
            self.wall_collision(axis)
    ...
    def wall_collision(self, axis):
        ...
        if self.exiting_nozzle(side, axis):  # Checking if the particle is going through the nozzle
