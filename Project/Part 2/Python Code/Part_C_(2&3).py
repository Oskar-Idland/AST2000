...
com = 1 / (star_mass + planet_mass) * (
            planet_mass * np.array([P_x0, P_y0]))  # Calculating the center of mass assuming star is in origin
...
def radial_velocity():
    t, r_Planet, v_Planet, r_Star, v_Star = integrate(T, dt, N, S_x0, S_y0, S_vx0, S_vy0, P_x0, P_y0, P_vx0, P_vy0, G,
    ...
    noise = np.random.normal(0, 1 / 5 * m,
                             N)  # Gaussian probability distribution with mean = 0 stddev = 1/5 of the max v_rad
