...
def vf_launch_res():
    """
    Verifying launch results using functions from the ast2000tools package
    :return: None
    """
...
def create_orbit_func(planet_idx):
    """
    Creates a function from numerically calculated positions by interpolating the positions
    :param planet_idx: Planet index
    :return: Functions for the position and velocity
    """
...
def locate_spacecraft(ref_p1_idx, ref_p2_idx, time):
    """
    Finds position of the spacecraft in the solar system using trilateration
    :param ref_p1_idx: Index of reference planet 1
    :param ref_p2_idx: Index of reference planet 2
    :param time: Time for the location process
    :return: Array with position of the spacecraft
    """
...
def calc_vel_dop(dopplershift, lambda0):
    """
    Calculates relative velocity based on the doppler shift
    :param dopplershift: Doppler shift
...
def find_velocity(lambda0, phi1, phi2, sc_ds1, sc_ds2, star_ds1, star_ds2):
    """
    Finds velocity in the solar system based on the light from reference stars
    :param lambda0: Reference wavelength
    :param phi1: Angle to first reference star from x-axis
    :param phi2: Angle to second reference star from x-axis
    :param sc_ds1: Doppler shift from reference star 1 measured by the spacecraft
    :param sc_ds2: Doppler shift from reference star 2 measured by the spacecraft
    :param star_ds1: Doppler shift from reference star 1 measured by at the star in our solar system
    :param star_ds2: Doppler shift from reference star 2 measured by at the star in our solar system
    :return:
    """
...
mission.take_picture()  # Taking picture
...
position_SC = locate_spacecraft(3, 6, utils.s_to_yr(885.21))  # Calculating position
velocity_SC = find_velocity(lambda0, phi1, phi2, sc_dop_shift1, sc_dop_shift2, star_dop_shift1, star_dop_shift2)  # Calculating velocity
angle_SC = find_angle("sky_picture.png")  # Measuring angle of heading of the spacecraft
print(f"Position: {position_SC}")
print(f"Velocity: {velocity_SC}")
print(f"Angle: {angle_SC}")

mission.verify_manual_orientation(position_SC, velocity_SC, angle_SC)  # Verifying orientation results
