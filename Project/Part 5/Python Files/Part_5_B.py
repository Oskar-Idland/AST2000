...
def check_close_enough():
    ...
    if min_distance < l:
        ...
        print('Close enough!!')
        plt.scatter(shuttle_position[int(index - time_index), 0], shuttle_position[int(index - time_index), 1],
                    label='Shuttle position close enough')
        plt.scatter(target_planet_trajectory[0, index], target_planet_trajectory[1, index],
                    label='Target planet position close enough')
        print(f'Total time of travel is {(final_time_index - index) * dt: .2f} years')
        print(f"Journey Start time: {index*dt}")
        print(f"Journey End time: {final_time_index*dt}")
        return True
...
if __name__ == "__main__":
    ...
    r_home_target = target_planet_trajectory[:, time_index] - home_planet_trajectory[:,
                                                              time_index]  # vector from home planet to target planet we can use to direct the initial velocity vector
    ...
    for angle in angles:
        for velocity in velocities:
            ...
            if check_close_enough():
                ...
                print(f"Initial Velocity: {velocity*np.array([np.cos(good_enough_angle), np.sin(good_enough_angle)])}")
