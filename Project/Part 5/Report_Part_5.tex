...
\addbibresource{Report_Part_5.bib}
\hypersetup{
    colorlinks,
...
\begin{figure}[h]
    ...
    \caption{Visualisation of a correctional boost to change the trajectory. The actual trajectory (black) deviates from the planned trajectory (red), then a trajectory
        correction $\Delta v$ is used to prevent the trajectory from drifting further away (blue). From~\parencite[][]{project_description5}
    }\label{fig:boost_fig}
\end{figure}
...
A description of the Leapfrog simulation method can be found in section II of the second paper~\parencite[][]{part2}
...
Using the orbits, calculated from the second paper~\parencite[][]{part2}
...
\subsection{Launching the Spacecraft}\label{subsec:launching-the-spacecraft}
    ...
    We then use the calculations from the first paper~\parencite[][]{part1}
